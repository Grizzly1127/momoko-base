#ifndef MOMOKOBASE_THREAD_POOL_SIMPLE_H
#define MOMOKOBASE_THREAD_POOL_SIMPLE_H
...
class ThreadPoolSimple : noncopyable
{
public:
    using Task = std::function<void()>;

    ThreadPoolSimple(size_t max_thread_size = 4);
    ~ThreadPoolSimple()
    {
        if (running_)
        {
